...
export class MailService {
  ...
  async sendUserCreatedEmail(
    ...
    try {
      ...
      console.log(
        `[MailService] ✅ User created email sent to ${email}:`,
        info.messageId,
      );
      return { success: true, messageId: info.messageId };
    } catch (error: any) {
  ...
  async sendPasswordResetEmail(
    ...
    try {
      ...
      console.log(
        `[MailService] ✅ Password reset email sent to ${email}:`,
        info.messageId,
      );
      return { success: true, messageId: info.messageId };
    } catch (error: any) {
  ...
  async sendPortalWelcomeEmail(
    ...
    console.log(
      `[MailService] 🌐 Portal Welcome - Email: ${email}, LoginID: ${loginId}, Password: ${password}, Type: ${contactType}`,
    );

    // Skip sending email if SMTP is not available
    if (!this.isMailEnabled || !this.transporter) {
      console.log(
        `[MailService] 📧 Portal welcome email skipped (SMTP not available) - credentials logged above`,
      );
      return { success: true, skipped: true };
    }

    const portalType = contactType === "CUSTOMER" ? "Customer" : "Vendor";

    try {
      const info = await this.transporter.sendMail({
        from: this.configService.get<string>("MAIL_FROM"),
        to: email,
        subject: `Welcome to SpendIQ ${portalType} Portal! 🎉`,
        html: `
          <!DOCTYPE html>
          <html>
            <head>
            ...
            <body>
              <div class="container">
                ...
                <div class="footer">
                  <p>This email was sent to ${email} because you were added as a ${contactType.toLowerCase()} in SpendIQ.</p>
                  <p>&copy; 2026 SpendIQ. All rights reserved.</p>
                </div>
              </div>
            </body>
      ...
      console.log(
        `[MailService] ✅ Portal welcome email sent to ${email}:`,
        info.messageId,
      );
      return { success: true, messageId: info.messageId };
    } catch (error: any) {
      console.error(
        `[MailService] ⚠️ Failed to send portal welcome email to ${email}. Credentials logged above.`,
        error,
      );
      return { success: false, error: error.message };
    }
