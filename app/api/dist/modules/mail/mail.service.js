...
let MailService = class MailService {
    constructor(configService) {
        ...
        let mailPassword = this.configService.get("MAIL_PASSWORD");
        ...
        if (mailHost && mailUser && mailPassword) {
            ...
            this.transporter.verify((error) => {
                if (error) {
                    ...
                    console.log(`[MailService] ✅ SMTP connected to ${mailHost}:${mailPort} as ${mailUser}`);
    ...
    async sendPortalInvitation(email, name, tempPassword) {
        console.log(`[MailService] 🔑 Portal Invitation for ${email} - Password: ${tempPassword}`);
        if (!this.isMailEnabled || !this.transporter) {
        ...
        `,
            ...
            console.error(`[MailService] ⚠️ Failed to send portal invitation to ${email}. Credentials logged above.`, error);
        }
    }
    async sendUserCreatedEmail(email, name, loginId, tempPassword, role) {
        console.log(`[MailService] 👤 User Created - Email: ${email}, LoginID: ${loginId}, Password: ${tempPassword}`);
        if (!this.isMailEnabled || !this.transporter) {
            console.log(`[MailService] 📧 User created email skipped (SMTP not available) - credentials logged above`);
            return { success: true, skipped: true };
        }
        try {
            const info = await this.transporter.sendMail({
        ...
        `,
            ...
            console.error(`[MailService] ⚠️ Failed to send user created email to ${email}. Credentials logged above.`, error);
            return { success: false, error: error.message };
        }
    }
    async sendPasswordResetEmail(email, name, loginId, newPassword) {
        console.log(`[MailService] 🔑 Password Reset - Email: ${email}, LoginID: ${loginId}, New Password: ${newPassword}`);
        if (!this.isMailEnabled || !this.transporter) {
            console.log(`[MailService] 📧 Password reset email skipped (SMTP not available) - credentials logged above`);
            return { success: true, skipped: true };
        }
        try {
            const info = await this.transporter.sendMail({
        ...
        `,
            ...
            console.error(`[MailService] ⚠️ Failed to send password reset email to ${email}. Credentials logged above.`, error);
            return { success: false, error: error.message };
        }
    }
};
